...
- ✅ **Gap 242: N+1 Query in Clip Retrieval**: Fetching a job with its associated clips triggered an N+1 database query problem instead of a `JOIN` or eager loading. (Fixed: Implemented `list_jobs_with_clips_for_campaigns()` in `db/repositories/jobs.py` using a single optimized `JOIN` query. Refactored `api/routes/campaigns.py` to batch-load all jobs and clips for the current page in a single call, reducing query count from O(N*M) to O(1) per page.)
- ✅ **Gap 243: Missing Circuit Breakers**: If the third-party transcription API goes down, the system continued to hammer it instead of tripping a circuit breaker and falling back gracefully. (Fixed: Implemented a global, distributed `RedisCircuitBreaker` in `core/redis_breaker.py`. Integrated `whisper_breaker` into `services/transcription.py`. When tripped, the entire worker fleet enters a 5-minute cooldown. `workers/pipeline.py` now catches `CircuitBreakerError` and automatically schedules a 10-minute exponential backoff retry for affected jobs.)
- ✅ **Gap 244: Synchronous Webhook Dispatch**: Webhooks were fired synchronously within the API request lifecycle instead of being deferred to a background task, blocking the caller. (Fixed: Verified all events in `services/event_emitter.py` use `.delay()` to queue `deliver_webhook_event` tasks in Celery. Integrated this into the `complete_job_atomic` flow to ensure webhooks only fire after successful DB commit.)
- ✅ **Gap 245: Lack of Schema Versioning**: The REST API did not use versioning (`/v1/`). Future changes to the Job or Clip schema would break older clients or external integrations. (Fixed: Refactored API architecture to use a centralized versioned router in `api/v1/router.py`. All endpoints migrated to the `/api/v1` prefix in `api/main.py`, providing a stable contract for external integrations and allowing side-by-side evolution of future schema versions.)
- ✅ **Gap 246: Bloated Event Payloads**: Redis PubSub messages carried the full JSON of the video object instead of just the ID, congesting the Redis network interface. (Fixed: Implemented "fetch-on-delivery" pattern in `workers/webhooks.py`. The emitter now passes a minimal snapshot, and the worker fetches rich data from the DB at delivery time. Includes a race-condition fallback that defaults missing fields to `null` and overrides `event_type` to `job.unavailable` if the record was deleted before delivery.)
- ✅ **Gap 247: Inefficient WebSockets Broadcast**: The WebSocket manager looped through all connections sequentially in Python instead of using Redis Pub/Sub directly to fan-out at the infrastructure level. (Fixed: Refactored `services/ws_manager.py` to use Redis Pub/Sub for fanning out events across distributed API nodes. Removed the 500ms polling loop in `api/routes/websockets.py` in favor of a non-blocking `pubsub.listen()` async task, ensuring near-zero latency for UI updates.)
- ✅ **Gap 248: Overlapping Background Beats**: Celery beat schedules for DB cleanup and Storage cleanup fired at the exact same midnight interval, causing unnecessary I/O spikes. (Fixed: Explicitly staggered all 24h periodic tasks in `workers/celery_app.py` using `crontab`. Cleanup now runs at 3 AM, Analytics at 1 AM/7 AM/1 PM/7 PM, and Strategy Summaries at 1:30 AM on Sundays.)
- ✅ **Gap 249: Unvalidated Enum States**: The database accepted raw strings for job states instead of enforcing an Enum constraint, risking invalid states like `processng` (typo). (Fixed: Added strict `CHECK` constraints to the `status` column in both `postgres_schema.sql` and `sqlite_schema.sql`. Valid states are now strictly enforced at the database level: `pending`, `processing`, `completed`, `failed`, `rejected`, `partial`.)
